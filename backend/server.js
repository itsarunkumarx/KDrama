...
app.get("/api/health", (req, res) =>
  res.json({ status: "ok", message: "KDrama Server Running 🎬" }),
);
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", ({ roomId, user }) => {
    ...
    const existing = watchRooms[roomId].users.find(
  ...
  socket.on("leave-room", ({ roomId, userId }) => {
    socket.leave(roomId);
    if (watchRooms[roomId]) {
      watchRooms[roomId].users = watchRooms[roomId].users.filter(
...
// ─── Start server (MongoDB removed) ───────────────────────────────────────
const PORT = parseInt(process.env.PORT, 10) || 5001;

server.listen(PORT, () => {
  console.log(`🚀 Server running on http://localhost:${PORT}`);
  console.log("✅ Running in DB-free mode (no MongoDB required)");
});

server.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    console.error(
      `❌ Port ${PORT} is already in use. Stop old backend process and retry.`,
    );
  } else {
