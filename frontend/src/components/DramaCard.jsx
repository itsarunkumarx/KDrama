...
export default function DramaCard({ drama, size = "md" }) {
  ...
  const isNew =
    ...
    new Date(drama.first_air_date) > new Date(Date.now() - 30 * 86400000);
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-2xl aspect-[2/3] bg-kcard shadow-lg hover:shadow-xl transition-shadow">
        <img
          src={imgError ? poster(null) : poster(drama.poster_path)}
          alt={drama.name}
        ...
        {/* Hover overlay */}
        <div
          className={`absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent transition-all duration-300 ${hovered ? "opacity-100" : "opacity-0"} flex flex-col justify-end`}
        >
      ...
      <div className="mt-3 px-1">
        <p
          className="text-white text-sm font-bold truncate leading-tight hover:text-pink-400 transition-colors cursor-pointer"
          onClick={() => navigate(`/watch/${drama.id}`)}
        >
          {drama.name}
        </p>
